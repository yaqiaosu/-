...
const Logo = () => {
  return (
    <div className="px-2">
      <svg viewBox="0 0 64 30" fill="#1772F6" width="64" height="30">
      ...
      {router.map((item, idx) => {
        ...
        return (
          <NavLink
            ...
            className={() => {
              return (
                'hover:text-black mx-4 py-2 transition-all flex items-center h-full' +
                (isActive ? ' text-black border-b-4 border-blue-600' : ' text-slate-500')
              );
            }}
          >
            {item?.title}
  ...
  return (
    <div className="flex mr-10 gap-6">
      <div className="flex justify-center items-center flex-col">
        ...
        <span className="text-sm text-slate-400">消息</span>
      </div>
      <div className="flex justify-center items-center flex-col">
        <ChatBubbleLeftRightIcon className="h-5 w-5 text-slate-400" />
        <span className="text-sm text-slate-400">私信</span>
      </div>
      <div className="flex justify-center items-center flex-col">
        <UserIcon className="h-5 w-5 text-slate-400" />
        <span className="text-sm text-slate-400">创作中心</span>
      </div>
...
export default function Navigation({ hideTabs }: { hideTabs: boolean }) {
  return (
    <div>
      {/* 第一个导航栏：固定在顶部，始终显示 */}
      <div className="fixed top-0 left-0 right-0 bg-white shadow h-12 box-border z-50">
      ...
      {/* 第二个导航栏：固定在第一个下面，显示时覆盖第一个 */}
      <div className={`fixed top-0 left-0 right-0 bg-white shadow h-12 box-border transition-all duration-300 ${hideTabs ? '-translate-y-full' : 'translate-y-0'}`}>
        <div className="h-12 flex justify-center">
          <div className="w-full max-w-7xl flex items-center justify-between min-w-6xl px-4">
            <div className="flex items-center">
              <Logo />
